...
class FeatureCrawl:
    ...
    def crawl(self, url):
        ...
        stats = {'paraCount':0,'smallParaCount':0,'wordCount':0,'imgCount':0,
                 ...
                 'listCount':0, 'listItemCount':0,'itemToListRatio':0,
...
#                     soup = BeautifulSoup(response.read())    
    ...
    #                 stats['imgCount'], stats['largeImgCount'] = self.countImages(siteurl, page_images)
                    ...
                    stats['listCount'], stats['listItemCount'] = self.countListsAndItems(soup)
                    ...
                    stats['itemToListRatio'] = round(stats['listItemCount']/float(stats['listCount']),2) if (stats['listItemCount']>0 and stats['listCount']>0) else 0
...
#                 print(link['href'])
    ...
    def countListsAndItems(self, soup):
        lists = soup.findAll('ul')
        li = soup.findAll('li')
        return len(lists), len(li)
